...
- Stop after ${this.maxQuestions} questions.`;
  ...
  async initializeInterview(domain, resumeText = '') {
    ...
    // ✅ Force intro opener (not from Gemini)
    const opener = "Hello, it's Gemini. Can you tell me a bit about yourself and your journey so far?";
    this.conversationHistory.push({ role: 'model', parts: [{ text: opener }] });
    return opener;
  }
   ...
   * @param {object} options { expectJson: boolean }
   */
  async sendMessage(userResponse = '', options = { expectJson: false }) {
    ...
    if (options.expectJson) {
      return await this._callGemini(this.conversationHistory, { forceJson: true });
    }

    // End → let InterviewFlow handle review
    if (this.questionCount >= this.maxQuestions && !this.awaitingRetry) {
    ...
    if (contents.length === 0) {
      ...
      contents.push({ role: 'user', parts: [{ text: this._phaseInstruction() }] });
    }

    const raw = await this._callGemini(contents);
    this.conversationHistory.push({ role: 'model', parts: [{ text: raw }] });
    return this._extractCoreQuestion(raw);
  }

  async _callGemini(contents, { forceJson = false } = {}) {
    const body = {
      contents,
      generationConfig: {
        temperature: 0.7,
    ...
    try {
      ...
      throw new Error("Invalid JSON response received from the Gemini API.");
    }
  }
  ...
  _extractCoreQuestion(response) {
    ...
    const sentences = cleaned.split(/(?<=[?.!])\s+/).map(s => s.trim());
    return sentences.find(s => s.endsWith('?')) || '';
  }

  _phaseInstruction() {
    switch (this.currentPhase) {
      case 'opening': return 'Ask a friendly question about their background or motivation.';
      case 'resume': return 'Ask about a project, skill, or achievement from their background.';
      case 'domain': return `Ask a practical question about ${this.domain}.`;
      case 'behavioral': return 'Ask about teamwork, leadership, or handling challenges.';
      case 'closing': return 'Ask about their future goals or if they have questions.';
      default: return 'Ask a clear, relevant interview question.';
    }
  }
}
